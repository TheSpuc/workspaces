...
<ul id="nav">
	...
	<li class="top"><?php echo $this->Html->link($this->Html->tag('span', 'Office', array('class' => 'down')), array('plugin' => null,'controller' => 'users', 'action' =>'office'), array('class' => 'top_link', 'id' => 'office', 'escape' => false)); ?>
		<ul class="sub">
			...
			<li class="mid"><a href="#nogo8" class="fly">Scouting</a>
				<ul>
				<?php
					echo '<li>' . $this->Html->link('Network', array('plugin' => null,'controller' => 'users', 'action' =>'scouting')) . '</li>';
					echo '<li>' . $this->Html->link('Assignments', array('plugin' => null,'controller' => 'ScoutAssignment', 'action' =>'overview')) . '</li>';
				?>
				</ul>
			</li>
		</ul>
