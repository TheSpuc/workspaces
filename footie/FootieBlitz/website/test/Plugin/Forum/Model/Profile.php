...
class Profile extends ForumAppModel {
	...
	public function increasePosts($user_id) {
		return $this->query('UPDATE '. $this->tablePrefix .'profiles SET "totalPosts" = "totalPosts" + 1 WHERE user_id = '. (int) $user_id);
	}
	...
	public function increaseTopics($user_id) {
		return $this->query('UPDATE '. $this->tablePrefix .'profiles SET "totalTopics" = "totalTopics" + 1 WHERE user_id = '. (int) $user_id);
	}
	
	public function setLastIndexView($user_id) {
		return $this->query('UPDATE '. $this->tablePrefix .'profiles SET lastindexview = now() WHERE user_id = '. (int) $user_id);
	}
