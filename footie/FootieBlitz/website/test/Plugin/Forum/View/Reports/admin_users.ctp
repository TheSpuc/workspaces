...
$this->Html->addCrumb(__d('forum', 'forumusers'), array('controller' => 'reports', 'action' => 'users')); ?>
				...
				<tr<?php if ($counter % 2) echo ' class="altRow"'; ?>>
					...
					<td>
						<?php if (!empty($report['User']['id'])) {
							echo $this->Html->link($report['User'][$config['userMap']['username']], array('controller' => 'forumusers', 'action' => 'edit', $report['User']['Profile']['id'], 'admin' => true));
						} else {
							echo '<em class="gray">('. __d('forum', 'Deleted') .')</em>';
						} ?>
					</td>
					<td><?php echo $this->Html->link($report['Reporter'][$config['userMap']['username']], array('controller' => 'forumusers', 'action' => 'edit', $report['Reporter']['Profile']['id'], 'admin' => true)); ?></td>
