			...
			<tr>
				...
				<td>
					<?php if (!empty($profile['Profile']['lastLogin'])) {
						echo 'site: ' . $this->Time->timeAgoInWords(date("Y-m-d H:m:s", $profile['User']['login']), array('userOffset' => $this->Common->timezone()));
						echo '<br />forum: ' . $this->Time->timeAgoInWords($profile['Profile']['lastLogin'], array('userOffset' => $this->Common->timezone()));
					} else {
...
if($profile['User']['id'] == $uid){
	echo '<h3>' . $this->Html->link(__d('forum', 'Edit Profile'), array('controller' => 'forumusers', 'action' => 'edit', 'admin' => false), array('class' => 'button float-right')) . '</h3><br/>';
}
		...
		<table class="table">
			...
			<?php foreach($posts as $post) { ?>
				...
				<tr class="altRow">
					...
					<td><?php echo $this->Html->link($post['Topic']['User'][$config['userMap']['username']], array('controller' => 'forumusers', 'action' => 'profile', $post['Topic']['User']['id'])); ?></td>
