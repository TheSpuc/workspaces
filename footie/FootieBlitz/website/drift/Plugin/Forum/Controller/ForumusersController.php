...
class ForumusersController extends ForumAppController {
	...
	public function edit() {
		...
		if (!empty($this->request->data)) {
			...
			//Find if the current password matches
			$userPass = false;
			$saveFail = false; //to see if a save fails, so the other saves shouldn't continue.
			if(!empty($this->request->data['Profile']['currentpassword'])){
				$pass = AuthComponent::password($this->request->data['Profile']['currentpassword']);
				...
				//If password matches
				if($userPass == true){
					//Are there new passwords and do they match?
					if(!empty($this->request->data['Profile']['newpassword']) && !empty($this->request->data['Profile']['repassword'])){
						if($this->request->data['Profile']['newpassword'] == $this->request->data['Profile']['repassword']){
							$userData = array(
								'User' => array(
									'password' => $this->request->data['Profile']['newpassword']
								)
							);
							//Save new passwords
							...
							if($this->User->save($userData)){
								$this->Session->setFlash(__d('forum', 'Your password has been updated'));
							}
							else{
								$this->Session->setFlash(__d('forum', 'Your password could not be updated'));
								$saveFail = true;
							}
						}
						else{
							$this->Session->setFlash(__d('forum', 'Your new passwords do not match'));
							$saveFail = true;
						}
					}
					//New email
					if(!empty($this->request->data['Profile']['email']) && $saveFail == false){
						$this->User->id = $user_id;
						$userData = array(
								'User' => array(
						...
						if($this->User->save($userData)){
							$this->Session->setFlash(__d('forum', 'Your email has been updated'));
						}
						else{
							$this->Session->setFlash(__d('forum', 'Your email could not be updated'));
							$saveFail = true;
						}
					}					
					//Profile options
					if ($this->Profile->save($this->request->data, true) && $saveFail == false) {
						$this->Session->setFlash(__d('forum', 'Your profile information has been updated!'));
					}
				}
			}
			else{
				$this->Session->setFlash(__d('forum', 'Remember to supply your current password.'));
			}			
		} else {
