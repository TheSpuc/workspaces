...
class Topic extends ForumAppModel {
	...
	public function add($data) {
		...
		if ($this->validates()) {
			...
			if (($secondsLeft = $this->checkFlooding($this->settings['topic_flood_interval'])) > 0 && !$isAdmin) {
				...
				$data['title'] = Sanitize::clean($data['title'], array('encode' => false));
	...
	/**
	* Get the latest topics by forum id.
	*
	* @access public
	* @param int $forum_id
	* @param int $limit
	* @return array
	*/
	public function getLatestByForum($forum_id, $limit = 5) {
		return $this->find('all', array(
	...
	public function increaseViews($id) {
		return $this->query('UPDATE '. $this->tablePrefix .'topics SET view_count = view_count + 1 WHERE id = '. (int) $id);
	}
