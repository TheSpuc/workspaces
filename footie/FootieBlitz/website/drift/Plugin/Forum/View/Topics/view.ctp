...
if (!empty($topic['Poll']['id'])) { ?>
	...
	<div id="poll" class="container">
		<div class="containerHeader">
			<h3><?php echo h($topic['Topic']['title']); ?></h3>
		</div>
				...
				<?php } else {
					...
					<tr<?php if ($counter % 2) echo ' class="altRow"'; ?>>
						<td class="dark align-right">
							<strong><?php echo h($option['option']); ?></strong>
						</td>
						<td style="width: 50%">
			...
			<tbody>
				...
				<tr>
					<td valign="top" style="width: 25%">
						<h4 class="username"><?php echo $this->Html->link($post['User'][$config['userMap']['username']], array('controller' => 'forumusers', 'action' => 'profile', $post['User']['id'])); ?></h4>
					...
					<td valign="top">
						<div class="post">
							<?php echo strip_tags($post['Post']['contentHtml'], '<strong><b><br><em><i><s><u><img><a><pre><li><ul><div><p><blockquote><span>'); ?>
						</div>

						<?php if (!empty($post['User']['Profile']['signatureHtml'])) { ?>
							<div class="signature">
								<?php echo h($post['User']['Profile']['signatureHtml']); ?>
							</div>
						<?php } ?>
					</td>
				</tr>
