...
<table>
	...
	<tr>
		<td>Name</td><td>
		<form method="POST" id="frmScoutPlayer">
		<?php 
		...
		<?php
		if ($scoutAssignments == 0){ 
			echo '<a href="#" id="lnkScoutPlayer">[scout]</a>';
			echo '<input type="hidden" name="sbmScoutPlayer" />';
		}
		else{
			echo '(currently scouting)';
		}
		?>
		</form>
		</td>
		...
		echo "</tr><tr><td>Agent</td><td>";
			// user.id 99 = NPC
     		if ($personUser['User']['id'] != 99){ 
				...
				echo 'No agent ';
				if ($agentContractOfferedID == 0){
					echo $this->Html->link('[offer representation]',  '/AgentContract/offer/' . $this->data['Person']['id']);
				}
				else{
					echo '- offer sent ' . $this->Html->link('[change offer]',  '/AgentContract/changeoffer/' . $agentContractOfferedID);
				}
			}
     		echo "</td></tr>";
