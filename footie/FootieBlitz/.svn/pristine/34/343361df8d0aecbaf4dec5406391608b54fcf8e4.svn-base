...
public class ClientApplet extends java.applet.Applet implements Runnable{
...
//			boldY = Integer.parseInt(s[i]) / SIZE_FACTOR;
			...
			try {
				if (getCodeBase().getHost().length() == 0){
					url = new URL("http://www.cake2.footie-online.com/matches/livedata/" + matchID + "/" + fr);
				}else{
					url = new URL("http://" + getCodeBase().getHost() + "/matches/livedata/" + matchID + "/" + fr);
				}
	...
	public void init() {
		...
		try {
			if (getCodeBase().getHost().length() == 0){
				...
				System.out.println("CB not found");
				url = new URL("http://www.cake2.footie-online.com/matches/livedata/" + matchID + "/" + fr);
			}else{
