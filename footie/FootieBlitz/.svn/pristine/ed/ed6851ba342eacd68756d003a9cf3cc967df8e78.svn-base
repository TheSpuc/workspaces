...
<div class="statistics">
    ...
    <?php if (!empty($newestUser)) { ?>
		<div class="newestUser">
			<strong><?php echo __d('forum', 'Newest User'); ?>:</strong> <?php echo $this->Html->link($newestUser['User'][$config['userMap']['username']], array('controller' => 'forumusers', 'action' => 'profile', $newestUser['User']['id'])); ?>
		</div>
   	<?php }
	...
	if (!empty($whosOnline)) {
		...
		foreach ($whosOnline as $online) {
			$onlineUsers[] = $this->Html->link($online['User'][$config['userMap']['username']], array('controller' => 'forumusers', 'action' => 'profile', $online['User']['id']));
		} ?>
