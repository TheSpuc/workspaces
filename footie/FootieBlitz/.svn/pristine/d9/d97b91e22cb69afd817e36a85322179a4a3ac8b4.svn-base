	...
	<td class="activity">
		<?php if (!empty($forum['LastTopic']['id'])) {
			...
			<?php if (!empty($forum['LastUser']['id'])) { ?>
				<span class="gray"><?php echo __d('forum', 'by'); ?> <?php echo $this->Html->link($forum['LastUser'][$config['userMap']['username']], array('controller' => 'forumusers', 'action' => 'profile', $forum['lastUser_id'])); ?></span>
			<?php }
		} else { ?>
