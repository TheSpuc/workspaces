...
<div class="title">
	...
	<?php if (!empty($forum['Forum']['description'])) { ?>
		<p><?php echo '<p>' . $forum['Forum']['description'] . '</p><br>'; ?></p>
	<?php } ?>
</div>
	...
	<div class="statistics">
		...
		if (!empty($forum['Moderator'])) {
			foreach ($forum['Moderator'] as $mod) {
				$moderators[] = $this->Html->link($mod['User'][$config['userMap']['username']], array('controller' => 'forumusers', 'action' => 'profile', $mod['User']['id']));
			}
		} ?>
