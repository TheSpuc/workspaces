...
public class Bold {
	...
	public void adjustAForCurl(Point target){
		...
		double boldY = getY();
		double boldA = getA();
		double boldSpeed = getSpeed();
		double boldZ = getZ();
		double boldZForce = getZForce();
		double boldCurl = getCurl();

		//Check the trajectory of the ball until it's further away than the target
		int whileLimit = 500;
		while (whileLimit > 0 && Hjaelper.Distance(boldX, boldY, startX, startY) < distance){
			whileLimit--;

			boldX += boldSpeed * Math.sin(boldA) * 0.03;
			boldY += boldSpeed * Math.cos(boldA) * 0.03;
			boldZ += boldZForce;
			...
			if (boldZ > 1){
				...
				boldZForce -= gravity / m;
				boldSpeed *= luftFaktor; //luftens modstand
				boldSpeed -= luftKonstant;
			}
			else{

				//pga banens modstand
				boldSpeed *= jordFaktor;
				boldSpeed -= jordKonstant;
				boldCurl *= 0.9;
			}

			//hvis bolden rammer jorden hopper den op igen
			if (boldZ < 0 && boldZForce < 0){
				boldZ = 0;
				boldZForce *= hoppeKonstant; //tag lidt af kraften ud af bolden og vend dens z-retning om
				boldSpeed *= 0.7;//tage lidt af farten på bolden når den rammer jorden
				boldCurl *= 0.5;//tag lidt af skrug af når bolden rammer jorden
			}
			if (boldZ <= 4 && boldZForce > 0 && boldZForce < 2){
				boldZForce = 0;
				boldZ = 0;
			}
			if (boldSpeed < 0) boldSpeed = 0;	
			if (boldX < 0) boldX = 0;
			if (boldY < 0) boldY = 0;

			boldA += boldCurl * boldSpeed / 100.0;
			boldCurl *= 0.98;
		}

		double angleDifference = Hjaelper.angleDifference(Math.atan2(target.x - getX(), target.y - getY()), 
				Math.atan2(boldX - getX(), boldY - getY()));

		System.out.println("adjust a angleDifference: " + angleDifference);
