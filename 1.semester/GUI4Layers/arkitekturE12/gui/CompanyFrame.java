...
/**
 * The CompanyFrame class implements a view that shows all companies. The view
 * also enables the user to open other views, where he can create, update or
 * delete a company.<br />
 * 
...
public class CompanyFrame extends JFrame {
	...
	public CompanyFrame() {
		...
		lstCompanies = new JList<Company>();
	...
	private class Controller implements ActionListener, ListSelectionListener {
		...
		public void fillLstCompanies() {
			lstCompanies.setListData(Service.getCompanies().toArray(new Company[0]));
		}
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnCreate) {
				CompanyCUDialog createDialog = new CompanyCUDialog(
						CompanyFrame.this, "Create New Company");
				...
				if (createDialog.isOKed()) {
					...
					lstCompanies.setSelectedIndex(lastIndex); // calls
																// updateView()
				...
				if (lstCompanies.getSelectedIndex() != -1) {
					...
					CompanyCUDialog updateDialog = new CompanyCUDialog(
							CompanyFrame.this, "Update Company");
					Company company = (Company) lstCompanies.getSelectedValue();
					...
					if (updateDialog.isOKed()) {
						...
						this.fillLstCompanies();
						lstCompanies.setSelectedIndex(selectedIndex); // calls
																		// updateView()
						lstCompanies.ensureIndexIsVisible(selectedIndex);

						// The EmployeeFrame view ought to be updated here too.
						// But it it not done.
						// To update the EmployeeFrame view the user must close
						// and reopen the view.
					}
				...
				if (index != -1) {
					...
					int answer = JOptionPane.showConfirmDialog(
							CompanyFrame.this, "Are you sure?",
							"Delete Company", JOptionPane.YES_NO_OPTION);
					if (answer == JOptionPane.YES_OPTION) {
						Company company = (Company) lstCompanies
								.getSelectedValue();
						if (company.getEmployeesCount() == 0) {
							...
							JOptionPane.showMessageDialog(CompanyFrame.this,
									"Can't delete a company with employees.",
