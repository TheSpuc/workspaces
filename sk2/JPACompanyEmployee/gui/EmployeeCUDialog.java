 ...
 * the Create button or the Update button. A new view object is created each
 * time the user clicks one of these buttons in the EmployeeFrame view.
 * 
 * @author mlch
 */
public class EmployeeCUDialog extends JDialog {
	...
	private JComboBox<Company> cbxCompanies;
	...
	private class Controller implements ActionListener {
		...
		public void fillCbxCompanies() {
			DefaultComboBoxModel<Company> cbxCompaniesModel = new DefaultComboBoxModel<Company>(
					service.getAllCompanies().toArray(new Company[0]));
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnOk) {
				...
				if (cbxCompanies.getSelectedIndex() >= 1) {
					company = (Company) cbxCompanies.getItemAt(cbxCompanies
							.getSelectedIndex());
				}
				...
				if (employee == null) {
					...
					if (company != null)
						service.updateCompanyOfEmployee(newEmployee, company);
				} else {
					if (!employee.getName().equals(name)
							|| employee.getWage() != wage)
						service.updateEmployee(employee, name, wage);
					if (employee.getCompany() != company)
						service.updateCompanyOfEmployee(employee, company);
				}
